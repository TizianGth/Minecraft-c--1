...
/*glm::vec3 cameraDirection = glm::normalize(cameraPos - glm::vec3(0, 0, 0));
...
void test::TestBatching::OnRender(int screenWidth, int screenHeight)
{
	...
	*/
	...
	m_Mvp = glm::translate(m_Proj * cam.m_Mat4 * m_Model, cameraPos);
...
void test::TestBatching::KeyboardMovement(float deltaTime)
{
	if (Input::IsKeyPressed(GLFW_KEY_W)) {
		cam.m_Position += cam.m_Direction * movementSpeed * deltaTime;
	}
	if (Input::IsKeyPressed(GLFW_KEY_S)) {
		cam.m_Position -= cam.m_Direction * movementSpeed * deltaTime;
	}
	if (Input::IsKeyPressed(GLFW_KEY_A)) {
		cam.m_Position -= glm::normalize(glm::cross(cam.m_Direction, cam.m_Up)) * movementSpeed * deltaTime;
	}
